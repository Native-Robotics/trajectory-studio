...
OUT=ROOT/'collections/barilla-curobo-continuous-production-20260913'
PAIRS=ROOT/'collections/barilla-curobo-reference-20260913'
RUN=BASE/'curobo-continuous-production-full-20260913'
...
def main():
    rows=read(OUT/'trajectories.json');comparison=read(OUT/'motion-comparison.json');assert len(rows)==270 and len({r['id'] for r in rows})==270
    ...
    for row in rows:
        ...
        expected_root=RUN/'Trajectories/traj' if row['pair_role']=='curobo' else PAIRS/'Trajectories/traj'
        assert dst.with_suffix('.repr').read_bytes()==(expected_root/(ident+'.repr')).read_bytes();count+=1
        expected=expected_root/(ident+'.traj')
    ...
    for name in ('index.html','report.html','trajectories.json','motion-comparison.json','exact-export-audit.json','benchmark/summary.json'):
