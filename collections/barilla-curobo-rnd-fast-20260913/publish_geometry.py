...
FINAL = BASE / 'curobo-rnd-original-geometry-full-20260913'
COLD = BASE / 'curobo-rnd-original-geometry-fresh10-20260913'
WARM = BASE / 'curobo-rnd-original-geometry-warm10-20260913'
...
def main():
    ...
    _, previous = report.measurements(BASE / 'curobo-rnd-final-20260913')
    _, cold_old = report.measurements(BASE / 'curobo-rnd-buffer256-fresh10-20260913')
    _, cold_new = report.measurements(COLD)
    _, warm_old = report.measurements(BASE / 'curobo-rnd-buffer256-warm-validation-20260913')
    ...
    report.NAME='barilla-curobo-rnd-fast-20260913'
    sys.argv=['publish_final.py','--replay',str(FINAL),'--cold',str(COLD),'--warm',str(WARM),'--note','Final integrated R&D replay. Primary comparison below uses the previous published R&D version. Historical develop and original GPU comparisons are retained separately and do not establish equal validity.']
    report.main()
    folder=report.STUDIO/'collections'/report.NAME
    evidence={'baseline':'curobo-rnd-final-20260913','candidate':FINAL.name,'metrics':groups,'warm_summaries':warm_summaries,'motion_rows':motion_rows}
    ...
    qualified = sum(r['status']=='SOLVED' and r['constraints_valid'] is True for r in final)
    fresh = next(r for r in groups if r['metric']=='Fresh planning latency (s)')
    warm = next(r for r in groups if r['metric']=='Retained-worker request latency (s)')
    intro = f"<h2>{qualified} / 135 recorded strict passes</h2><p>Fresh computation: <strong>{fresh['current']:.2f} s</strong> vs {fresh['previous']:.2f} s. Repeated requests: <strong>{warm['current']:.2f} s</strong> vs {warm['previous']:.2f} s.</p>"
    section = intro + section
    page=folder/'report.html'
    page.write_text(page.read_text().replace('</h1>','</h1>'+section,1))
    md=folder/'report.md'
    md.write_text('# Faster preparation versus previous published cuRobo R&D\n\n'+ '\n'.join(f"- {r['metric']}: {r['previous']:.6g} → {r['current']:.6g} ({r['pairs']} paired tasks)" for r in groups)+'\n\nSee strict-comparison.json for regressions and per-task metrics.\n\n'+md.read_text())
