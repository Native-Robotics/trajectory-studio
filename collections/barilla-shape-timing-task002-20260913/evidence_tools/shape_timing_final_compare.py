...
OUT=BASE/'curobo-shape-timing-final-task002-20260913';OUT.mkdir(exist_ok=True)
...
report={}
variants=[('ref','#ef8b23',next(COL.glob('236*-ref.traj'))),('curobo','#00a6c8',next(COL.glob('236*-curobo.traj'))),('retimed','#398c48',BASE/'curobo-shape-timing-sweep-task002-20260913/candidate.traj'),('smooth-join','#9862bd',BASE/'curobo-shape-join-task002-20260913/anchors3-gain0.7.traj')]
for variant,color,source in variants:
 data=json.loads(source.read_text());rows=[];offset=0;end_tangents=[]
 for i,(part,d) in enumerate(zip(task.parts,data['parts'])):
  ...
  dist=np.linalg.norm(p-p[0],axis=1);first=np.flatnonzero(dist>=.001)[0];distend=np.linalg.norm(p-p[-1],axis=1);last=np.flatnonzero(distend>=.001)[-1];end_tangents.append((p[first]-p[0],p[-1]-p[last]));ranges=_derivative_ranges(s);jp=np.max(np.abs(ranges),axis=-1)
  duration=t[-1]-t[0]
 ...
 angles=[float(np.degrees(np.arccos(np.clip(a[1]@b[0]/np.linalg.norm(a[1])/np.linalg.norm(b[0]),-1,1)))) for a,b in zip(end_tangents,end_tangents[1:])];report[variant]={'duration_s':offset,'parts':rows,'join_tangent_angle_deg_over_1mm':angles}
