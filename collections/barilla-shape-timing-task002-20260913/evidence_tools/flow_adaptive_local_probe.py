...
import time
OUT=BASE/'curobo-shape-adaptive-local-task002-20260913';OUT.mkdir(exist_ok=True)
...
rows=[]
unrestricted=res.x.copy()
for join_factor in (.55,.7,.85):
 ramp=.4;sigma=1.
 profile=np.ones_like(s)
 for boundary in sb[1:-1]:profile*=1-(1-join_factor**2)*np.exp(-((s-boundary)/.10)**2)
 local_bounds=np.minimum(bound[1:-1],unrestricted*profile[1:-1])
 res=minimize(objective,np.minimum(unrestricted,local_bounds),jac=jac,bounds=[(1e-8,b) for b in local_bounds],constraints={'type':'ineq','fun':con,'jac':conjac},method='SLSQP',options={'maxiter':150,'ftol':1e-8})
 x=np.r_[0,res.x,0];regular_s=np.linspace(0,L,1801);regular_x=np.interp(regular_s,s,x);regular_x=gaussian_filter1d(regular_x,10,mode='constant');x=np.interp(s,regular_s,regular_x);x[[0,-1]]=0
 ...
 final=retime_solution(ComplexSolution(sols),factor*1.02);row={'join_factor':join_factor,'duration_s':float(total*factor*1.02),'time_scales_by_constraint':causes}
 try:
  ...
  if row['collision_valid']:
   ...
   name=f'adaptive-local-{join_factor:g}.traj';(OUT/name).write_text(json.dumps({'parts':parts}));row['candidate']=name
