...
import time
OUT=BASE/'curobo-shape-adaptive-task002-20260913';OUT.mkdir(exist_ok=True)
...
for sigma in (1.,2.,4.,6.):
 ...
 t=np.r_[0,np.cumsum(2*ds/(np.sqrt(x[:-1])+np.sqrt(x[1:])))];inverse=PchipInterpolator(t,s)
 tb=np.interp(sb,s,t);tt=[];qt=[]
 for i in range(3):
  times=np.linspace(tb[i],tb[i+1],181);ss=inverse(times);qq=qgeom(ss);qq[0]=task.parts[i].start.position.value;qq[-1]=task.parts[i].target.position.value
 ...
 final=retime_solution(ComplexSolution(sols),factor*1.02);row={'sigma':sigma,'duration_s':float(t[-1]*factor*1.02),'time_scales_by_constraint':causes}
 try:
  ...
  if row['collision_valid']:
   ...
   name=f'adaptive-{sigma:g}.traj';(OUT/name).write_text(json.dumps({'parts':parts}));row['candidate']=name
